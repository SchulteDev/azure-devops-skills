...
- **Artifact downloads:** on the local MCP server, `pipelines_artifact` (action `download`) reports success but writes a **0-byte file for pipeline artifacts** (`resource.type` `PipelineArtifact` in action `list`); classic `Container` artifacts download correctly (microsoft/azure-devops-mcp#1496). A relative `destinationPath` resolves against the MCP server's working directory. Check the file size, and for pipeline artifacts use `az pipelines runs artifact download --run-id <buildId> --artifact-name <name> --path <dir>`.
